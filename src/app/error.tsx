...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div style={{ maxWidth: 'var(--max-width)', margin: '0 auto', padding: 'var(--space-16) var(--space-4)', textAlign: 'center' }}>
      ...
      <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'var(--text-2xl)', fontWeight: 700, color: 'var(--maroon)', marginBottom: 'var(--space-2)' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'center' }}>
        <button onClick={reset} style={{ padding: 'var(--space-3) var(--space-8)', fontWeight: 600, color: 'white', background: 'var(--maroon)', borderRadius: 'var(--radius-full)', cursor: 'pointer' }}>
          Try Again
        </button>
